...
####################Build info diagramm #######################
plt.clf()
plt.bar(MyNetwork.info.keys(), MyNetwork.info.values())
plt.savefig('output/port_count.png')
...
print('--------------------')
#print(X_dist)
clustering = DBSCAN(eps=1.5, min_samples=1).fit(x_pca)
print("======================")
plt.clf()
...
T = hierarchy.fcluster (mergins, t=6, criterion='maxclust')
...
Create_text_files(Old, T, "hierarch") # starts from 1 func needs optimization
...
c.close()
##################Write undecoded#######################
d = open("output/Decode.txt", 'w')
for cluster in MyNetwork.data:
    d.write(str(cluster) + "\n")
d.close()
